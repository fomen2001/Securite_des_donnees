...
#
# Prérequis :
# pip install streamlit pandas numpy matplotlib statsmodels scikit-learn openpyxl
...
if use_sample:
    ...
    df = pd.DataFrame(
        {
            "Date": rng,
            "CO(GT)": 2 + 0.002 * t + 0.6 * np.sin(2 * np.pi * t / 7) + 0.3 * np.sin(2 * np.pi * t / 365.25)
...
try:
    ...
    axes[0].plot(stl_res.observed); axes[0].set_title("Observed")
    axes[1].plot(stl_res.trend); axes[1].set_title("Trend")
    axes[2].plot(stl_res.seasonal); axes[2].set_title("Seasonal")
    axes[3].plot(stl_res.resid); axes[3].set_title("Resid")
    for ax in axes:
        ax.grid(True, alpha=0.3)
    st.pyplot(fig, clear_figure=True)
except Exception as e:
...
# Calcul "brut" (heuristique)
sd_raw = suggest_d_by_adf(y, max_d=2)
spq_raw = suggest_pq_from_acf_pacf(y, max_lags=int(max_lags_user))

# Saison suggérée : par défaut la période STL (modifiable plus bas dans SARIMA)
...
# Caps (parsimonie) — réglables par l'utilisateur
...
if run:
    try:
        ...
        if model_type == "ARIMA":
            ...
            y_fore = res.get_forecast(steps=horizon, exog=exog_future).predicted_mean if exog_cols else res.get_forecast(steps=horizon).predicted_mean
        ...
        future_index = pd.RangeIndex(start=0, stop=horizon, step=1) if used_freq is None else pd.date_range(start=last_idx, periods=horizon + 1, freq=used_freq)[1:]
