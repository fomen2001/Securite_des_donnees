...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item active">{{ $utilisateur->name }}</li>
@endsection
...
<div class="card shadow-sm border-0">
    <div class="card-header bg-white fw-semibold">
        <i class="bi bi-person-gear me-1"></i>Modifier — {{ $utilisateur->name }}
    </div>
    <div class="card-body">
        <form method="POST" action="{{ route('admin.utilisateurs.update', $utilisateur) }}">
            ...
            <div class="mb-3">
                ...
                <select name="role" class="form-select @error('role') is-invalid @enderror" required>
                    ...
                    @php
                        ...
                        $selected = old('role', $utilisateur->roles->first()?->name) == $role->name ? 'selected' : '';
                    @endphp
                    <option value="{{ $role->name }}" {{ $selected }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                <label class="form-label fw-semibold">Nouveau mot de passe
